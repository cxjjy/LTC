...
import { projectDeliveryModeOptions, projectRegionOptions } from "@/modules/projects/constants";
...
export function buildProjectFields(opportunityOptions: SelectOption[]): EntityFormField<CreateProjectDto>[] {
  return [
    ...
    { name: "deliveryMode", label: "交付模式", type: "select", options: projectDeliveryModeOptions },
    { name: "region", label: "所属区域", type: "select", options: projectRegionOptions },
    { name: "ownerName", label: "项目负责人", type: "text" },
    { name: "status", label: "状态", type: "select", options: projectStatusOptions },
